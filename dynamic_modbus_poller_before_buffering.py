...
logger = logging.getLogger("bbj-sense-gateway")
...
def create_serial_client(
    ...
    return ModbusSerialClient(
        ...
        baudrate=int(
            connection.get("baudrate") or 9600
        ),
        parity=normalize_parity(
            connection.get("parity")
        ),
        stopbits=normalize_stop_bits(
            connection.get("stop_bits")
        ),
        bytesize=8,
...
def read_modbus_registers(
    ...
    if function_code == 3:
        ...
        raise ValueError(
            "Only Modbus function codes 3 and 4 "
...
def run_device(
    ...
    communication_type = str(
        device.get("communication_type") or ""
    ...
    if communication_type not in {
        ...
        logger.warning(
            "Skipping unsupported communication type "
            "for device %s: %s",
            device.get("device_name"),
            communication_type,
        )
