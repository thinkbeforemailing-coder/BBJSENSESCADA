...
**2026-08-08 update:** the dead-code question flagged during the systematic audit (`frequency.py`'s POST endpoint) was resolved — confirmed it had zero callers anywhere (the frontend's frequency page reads live data from the generic telemetry-history endpoint instead), backed up its 285 rows, and removed the router/model/table entirely (backend commit `51e575c`). Not a security finding, just cleanup — noted here since it was tracked alongside the audit.

**2026-08-08, same day — tenant-scoping gap closed:** the other open item from the systematic audit, `gateway_alarm_events` and `notification_history` having no `tenant_id` column, was fixed (backend commit `5ead60e`). This platform is single-tenant today so nothing was actively exploitable, but `GET /gateway/alarms/active`, `GET /gateway/alarms/history/{gateway_id}`, `POST /gateway/alarms/{alarm_id}/acknowledge`, and `GET /notifications/history` were all permission-gated only, not tenant-filtered — any authenticated user could see or acknowledge any tenant's gateway alarms. Worse, `send_alarm_notification()` notified every user in the system on every alarm regardless of tenant, so this wasn't just a read-path gap. Added `tenant_id` (FK, NOT NULL, indexed) to both tables, backfilled with 100% coverage from existing relations (`gateway_credentials` for alarms, `alarm_events → devices` for notifications), and scoped all 4 endpoints plus the notification-dispatch loop by `current_user["tenant_id"]`, matching the pattern `devices.py` already used. Verified live: migration applied cleanly, zero-null backfill confirmed, ruff/pytest clean, monitored soak showed normal telemetry/alarm traffic with no errors post-restart.

**Additional finding, same day, not in the original list:** an orphaned Docker container (`bbj-postgres`, `timescale/timescaledb:latest-pg16`, created 2026-07-13 — predates the native Postgres instance the app actually uses) was found still running and explicitly publishing Postgres to `0.0.0.0:5432` at the Docker level. Same database/user names as production (`bbjsense`/`bbjadmin`) and the *same* weak `ChangeThis@123` password. Confirmed via read-only query it held nothing of value (one leftover bootstrap `admin`/`admin` row, no real schema — not a live copy of production data). Stopped, removed, and its volume deleted. Also swept the rest of the box for other externally-exposed ports (`ss -tlnp` + `Test-NetConnection` from outside): SSH (22) and Caddy (80/443) are the only ones actually reachable from the internet; the backend's own port 8000, the real Postgres (5433, loopback-only anyway), and the Google Ops Agent's ports (20201/20202) are all either not bound externally or blocked by the firewall with no exposure.
